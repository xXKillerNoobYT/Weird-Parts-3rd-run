...
class ReceiptVerificationTest(unittest.TestCase):
    def records(self):
        source = dict(format=2, content={'jobs': [{'job_number': 'SYN-42'}], 'job_lines': [{'needed_qty': 7}]}, run='test', localRole='sender', root='/synthetic/source',
        ...
        return [copy.deepcopy(record) for record in [source, source, before, after]]
    ...
    def test_unprojected_content_changes_fail(self):
        for target in [1, 3]:
            records = self.records()
            records[target]['content']['jobs'][0]['job_number'] = 'corrupted'
            with self.assertRaises(module.ValidationError):
                module.verify(*records)
            records = self.records()
            records[target]['content']['job_lines'][0]['needed_qty'] = 999
            with self.assertRaises(module.ValidationError):
                module.verify(*records)

    def test_optimized_python_still_rejects_invalid_receipts(self):
        with tempfile.TemporaryDirectory() as root:
            path = Path(root) / 'receipt.json'
            path.write_text(json.dumps({'run': 'x'}))
            result = subprocess.run([sys.executable, '-B', '-O', module.__file__,
                                     *([str(path)] * 4)], capture_output=True, text=True)
